...
function loadHistory() {
    $.post('/index.php', { action: 'get_data' }, function(data) {
...
function loadStatus() {
    $.post('/index.php', { action: 'get_status' }, function(data) {
        if (data.recording) {
...
function sendAction(action) {
    $.post('/index.php', { action: action }, function(data) {
        if (data.success) {
            if (action === 'clear') {
                currentData = [];
                updateUI();
...
$(document).ready(function() { 
    ...
    $('#exportBtn').click(() => window.location.href = '/export');
});